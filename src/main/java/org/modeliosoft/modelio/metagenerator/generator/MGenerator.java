...
public class MGenerator {
    ...
    public StringBuffer createPackage(Package mPackage) {
        ...
        result.append("MPackage " + IdGenerator.createId(mPackage) + " = new MPackage(\"" + mPackage.getUuid().toString() + "\",\"" + mPackage.getName() + "\",\"" + mPackage.getUuid().toString() + ".exml\");\n");

        if (mPackage.getOwner().equals(this.metamodelRoot)) {
            result.append("this.mPackages.add(" + IdGenerator.createId(mPackage) + ");\n");
        } else {
            result.append(IdGenerator.createId(mPackage.getOwner()) + ".getMPackages().add(" + IdGenerator.createId(mPackage) + ");\n");
        }
    ...
    public StringBuffer createClass(Class mClass) {
        ...
        result.append("MClass " + IdGenerator.createId(mClass) + " = new MClass(\"" + mClass.getUuid().toString() + "\",\"" + mClass.getName() + "\",\"" + mClass.getUuid().toString() + ".exml\");\n");

        result.append(IdGenerator.createId(mClass.getOwner()) + ".getMClass().add(" + IdGenerator.createId(mClass) + ");\n");
        result.append("this.mClass.put(\""+mClass.getName()+"\","+ IdGenerator.createId(mClass)+");\n");
    ...
    public StringBuffer createAttribute(Attribute mAttr) {
        ...
        result.append("MAttribute " + IdGenerator.createId(mAttr) + " = new MAttribute(\"" + mAttr.getUuid().toString() + "\",\"" + mAttr.getName() + "\",\"" + mAttr.getOwner().getUuid().toString() + ".exml\",this.mBaseTypes.get(\"" + mAttr.getType().getName() + "\"),false,true,false);\n");
        result.append(IdGenerator.createId(mAttr.getOwner()) + ".getMAttributes().add(" + IdGenerator.createId(mAttr) + ");\n");
    ...
    private StringBuffer createGeneralization(Class mClass) {
        ...
        for (Generalization gen : mClass.getParent()) {
            result.append("this.mClass.get(\"" + mClass.getName()+"\").getMSuperType().add(this.mClass.get(\"" + gen.getSuperType().getName()+"\"));\n");
        }

        for (Generalization gen : mClass.getSpecialization()) {
            result.append("this.mClass.get(\"" + mClass.getName()+"\").getMSubTypes().add(this.mClass.get(\"" + gen.getSubType().getName()+"\"));\n");
        }
        return result;
    }
    ...
    private StringBuffer createDependency(AssociationEnd mEnd) {
        StringBuffer result = new StringBuffer();
        result.append("\n// dependency " + mEnd.getOwner().getName() + "." + mEnd.getName() + "\n");
        ...
        if (idtarget != null) {
            result.append("MDependency " + IdGenerator.createId(mEnd) + " = new MDependency(\"" + mEnd.getUuid().toString() + "\",\"" + mEnd.getName() + "\",\"" + mEnd.getOwner().getUuid().toString() + ".exml\",this.mClass.get(\"" + idtarget + "\")," + mEnd.getMultiplicityMax().equals("1") + ",false,false," + mEnd.getAggregation().equals(AggregationKind.KINDISCOMPOSITION) + ");\n");
            result.append("this.mClass.get(\"" + mEnd.getOwner().getName() + "\").getMDependencys().add("+ IdGenerator.createId(mEnd) +");\n");
        }
        return result;
    }
